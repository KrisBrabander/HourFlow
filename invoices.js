...
const invoiceGenerator = {
    ...
    addToRevenue: function(id) {
        ...
        // Check if we have a revenue array in app.state
        if (!app.state.revenue) {
            app.state.revenue = [];
        }
        
        // Check if invoice is already in revenue
        if (app.state.revenue.some(item => item.invoiceId === id)) {
        ...
        app.state.revenue.push(revenueItem);
        
        // Save to localStorage
        app.saveData('revenue', app.state.revenue);
        
        // Ook direct in localStorage opslaan voor zekerheid
        try {
            localStorage.setItem('revenue', JSON.stringify(app.state.revenue));
            console.log('Revenue saved directly to localStorage');
        } catch (e) {
